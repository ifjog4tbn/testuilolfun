...
function Luna:CreateWindow(WindowSettings)
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "Argus X" and Release or "Public Version"
...
local function ValidateKey(userKey)
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		HomeTabSettings = Kwargify({
			Icon = 1,
			SupportedExecutors = {"Vega X", "Delta", "Nihon", "Xeno", "Volcano", "Swift"}, -- THESE DEFAULTS ARE PLACEHOLDERS!! I DO NOT ADVERTISE THESE, THEY ARE JUS THE FIRST THAT CAME TO MIND. I HAVE NO IDEA WHETHER THEYA RE RATS (they prob are) AND IM NOT RESPONSIBLE IF U GET VIRUSES FROM INSTALLING AFTER SEEING THIS LIST
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "Argus Interface Suite - Debugging Mode" break end
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("Argus X | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("Argus X | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Argus X | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("Argus X | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("Argus X | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Argus X | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Argus X | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
...
--[[if isStudio then
    ...
    t1:CreateParagraph({Text = "Welcome to the Argus X. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. Luna was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Luna Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Luna Today at " .. website})
